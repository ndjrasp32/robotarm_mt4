# 2026-06-11 MT4 Stage 0 학습 산출물 인덱스

## 한국어

이 문서는 2026-06-11 MT4 coordinate curriculum의 Stage 0 변경, 학습 결과, 영상 산출물을 묶어 보는 인덱스다. 모든 실행은 IsaacLab simulation이며 실제 MT4 로봇 motion은 실행하지 않았다.
...
This record indexes the 2026-06-11 MT4 coordinate curriculum Stage 0 artifacts after switching away from a top-down vertical-entry assumption. All runs were IsaacLab simulation only; no real MT4 robot motion was executed.

The latest Stage 0 run replaced the strict lateral latch with a workspace-entry phase gate. Final `workspace_entry_success_rate` was `0.4839`, `workspace_entry_radius_rate` was `0.9216`, and `target_three_camera_visible_rate` was `0.7795`. Stage 1 3x3 plane can now be attempted from this checkpoint, with gripper-camera visibility as the first likely improvement target if bootstrap fails.
