...
if [[ -z "${CHECKPOINT}" || ! -f "${CHECKPOINT}" ]]; then
  ...
  echo "        Set MT4_CHECKPOINT=/path/to/model.pt or run Stage 0 training first."
  exit 1
fi

echo "[INFO] Recording 60s coordinate Stage 0 demo"
echo "[INFO] task=${TASK_NAME}"
echo "[INFO] checkpoint=${CHECKPOINT}"
echo "[INFO] random targets come from the environment reset sampler"
