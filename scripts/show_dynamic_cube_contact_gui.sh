...
if [[ "${HEADLESS}" == "0" ]] && command -v xdpyinfo >/dev/null 2>&1 && ! xdpyinfo -display "${DISPLAY}" >/dev/null 2>&1; then
...
echo "[INFO] Opening dynamic cube contact GUI on DISPLAY=${DISPLAY}"
./isaaclab.sh -p "${PROJECT_DIR}/tools/show_dynamic_cube_contact_gui.py" "$@"
